...
\begin{frame}{Optimality Conditions --- NCD-Corrected Approach}
    ...
    \only<4, 5, 6>{
        \textbf{Problem:} $V_N^{pr}, V_N^{du}$ must not be good approximation spaces for the sensitivities.

        \only<5, 6>{
            \textbf{Solution:} Compute the approximated sensitivities in $V_N^{pr, d_{\mu_i}}, V_N^{du, d_{\mu_i}} \subseteq V$.

            \only<6>{
                \begin{block}{Approximate Sensitivities}
                    We define the approximate sensitivities as solutions to
                    \begin{align*}
    ...
    \only<7>{
        \begin{align*}
            a_\mu(\tilde{d}_{\mu_i} u_{N, \mu}, v) &= \partial_\mu Res_\mu^{pr}(u_{N, \mu})[v] \cdot e_i &&\forall v \in V_N^{pr, d_{\mu_i}}, \\
            a_\mu(q, \tilde{d}_{\mu_i} p_{N, \mu}) &= \partial_\mu Res_\mu^{du}(u_{N, \mu}, p_{N, \mu})[q] \cdot e_i + 2 k_\mu(q, \tilde{d}_{\mu_i} u_{N, \mu}) &&\forall q \in V_N^{du, d_{\mu_i}}.                    
        \end{align*}

        \begin{block}{Inexact Gradient of NCD-Corrected Functional}
            We get for the approximate sensitivities:
            \begin{align*}
...
\begin{frame}{\textit{a posteriori} Error Estimates}
    ...
    \only<3>{
        \begin{equation*}
            \abs{\mathcal{J}(u_\mu, \mu) - J_N(\mu)} \leq \Delta_{J_N}(\mu), \qquad \abs{\mathcal{J}(u_\mu, \mu) - \mathcal{J}_N(\mu)} \leq \Delta_{\mathcal{J}_N}(\mu)
        \end{equation*}
